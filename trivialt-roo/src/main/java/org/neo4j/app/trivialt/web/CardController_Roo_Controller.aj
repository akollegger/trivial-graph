...
privileged aspect CardController_Roo_Controller {
    ...
    public String CardController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        if (page != null || size != null) {
            ...
            uiModel.addAttribute("cards", Card.findPaged(page == null ? 0 : (page.intValue() - 1) * sizeNo, sizeNo));
            float nrOfPages = (float) Card.count() / sizeNo;
    ...
    public Collection<Proposal> CardController.populateProposals() {
        return Proposal.findAll();
    }
    
    @ModelAttribute("rounds")
    public Collection<Round> CardController.populateRounds() {
